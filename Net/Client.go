...
func (self *Client) Connect() error {
	...
	case ProtocolType_TCP:
		...
		self.Session = NewSession(&self.Chan_Packet, conn, self.protocol)
...
func (self *Client) Send(packet *Packet) (int, error) {
	//TODO:存储一个队列用于重连后重发
	//logs.Printf("client=%p,id=%d,session=%p,pkt=%p", &self, self.id, self.session, packet)
	if self.Session == nil {
...
func (self *Client) _ProcessPacket(packet *Packet) error {
	...
	//logs.Printf("%p,id=%s,Get packet:%d",self,self.Id,packet.MessageId)
	return nil
}

func (self *Client) IsConnected() bool {
...
func (self *Client) OnSessionClose(session *Session) {
	...
	if (self.Chan_Close != nil) {self.Chan_Close <- session}
	if self.AutoReconnect {
		self.chan_nextconnect = time.After(time.Second * 5)
	}
}
...
func (self *Client) Run() {
	...
	for {
		...
		case <-self.chan_nextheartbeat:
			if self.Session != nil {self.Session.Send(NewPacket(0, 0))}
			self.chan_nextheartbeat = time.After(time.Second * 10)
		default:
...
func (self *Client) SendGMJsonMsg(messageid uint32, errcode uint32, jsonMsg interface{}) (int, error) {
	return self.Session.SendGMJsonMsg(messageid, errcode, jsonMsg)
}
